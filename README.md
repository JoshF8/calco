...
- **Static app** — the Go core compiles to WASM, so import + generate run in the browser and the whole app deploys to GitHub Pages
- **Docs** — brand system, architecture doc, 7 ADRs
...
| In-browser | Go core compiled to WebAssembly — import + generate run entirely on the client, so the app is a static site (GitHub Pages) |
...
- [x] In-browser engine: domain core compiled to WASM, deployed to GitHub Pages — see [ADR-0007](docs/adr/0007-wasm-engine.md)
...
Open http://localhost:5173 and press **Import** to load a local Terraform repository (folder picker). The importer runs entirely in-process — actually in-process *in the browser*: the Go core is compiled to WebAssembly and loaded as a static asset, so no Docker, no network, no `terraform init`, and you can keep the server stopped.

The same static build is what GitHub Pages serves. `task dev` and `task build` rebuild the engine automatically (it's a gitignored artefact); the [pages](.github/workflows/pages.yml) workflow builds web + engine in CI and deploys. The live demo lives at https://joshf8.github.io/calco.
...
│   │   ├── cmd/wasm/               # the domain core compiled for the browser
│   │   └── internal/               # config, domain, application, adapters
│   └── web/                        # React frontend (feature-based)
│       ├── public/                 # calco.wasm + wasm_exec.js (built, gitignored)
│       └── src/
│           ├── features/           # canvas, generate, import, catalog…
│           └── lib/wasm-core.ts    # boots the in-browser engine
├── docs/
...
- [`docs/adr/`](docs/adr/) — architecture decision records (7 so far)
